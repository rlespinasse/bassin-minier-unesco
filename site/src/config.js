...
export const layerGroups = [
  {
    group: 'Patrimoine mondial UNESCO',
    layers: [
      { id: 'batis', label: 'Batis', file: 'data/batis.geojson', active: true },
      {
        id: 'bien-inscrit',
        label: 'Bien inscrit',
        file: 'data/bien-inscrit.geojson',
        active: true,
      },
      { id: 'cavaliers', label: 'Cavaliers', file: 'data/cavaliers.geojson', active: true },
      {
        id: 'cites-minieres',
        label: 'Cites minieres',
        file: 'data/cites-minieres.geojson',
        active: true,
      },
      {
        id: 'espace-neonaturel',
        label: 'Espaces neo-naturels',
        file: 'data/espace-neonaturel.geojson',
        active: true,
      },
  ...
  {
    group: 'Autres \u00e9l\u00e9ments du bassin minier',
    layers: [
      {
        id: 'zt-cavaliers',
        label: 'Cavaliers (zone tampon)',
        file: 'data/zt-cavaliers.geojson',
        active: false,
      },
      {
        id: 'zt-cites-minieres',
        label: 'Cites minieres (zone tampon)',
        file: 'data/zt-cites-minieres.geojson',
        active: false,
      },
      {
        id: 'zt-espaces-neonaturels',
        label: 'Espaces neo-naturels (zone tampon)',
        file: 'data/zt-espaces-neonaturels.geojson',
        active: false,
      },
      {
        id: 'zt-terrils',
        label: 'Terrils (zone tampon)',
        file: 'data/zt-terrils.geojson',
        active: false,
      },
      {
        id: 'zt-parvis-agricoles',
        label: 'Parvis agricoles (zone tampon)',
        file: 'data/zt-parvis-agricoles.geojson',
        active: false,
      },
      {
        id: 'puits-de-mines',
        label: 'Puits de mines',
        file: 'data/puits-de-mines.geojson',
        active: false,
      },
    ],
  },
...
export const contextLayers = [
  {
    id: 'bassin-minier',
    label: 'Bassin minier (ERBM)',
    file: 'data/bassin-minier.geojson',
    active: true,
  },
  ...
  { id: 'epci', label: 'Intercommunalites (EPCI)', file: 'data/epci.geojson', active: false },
  {
    id: 'departements',
    label: 'D\u00e9partements',
    file: 'data/departements.geojson',
    active: false,
  },
];
...
export const searchableProps = {
  ...
  'bien-inscrit': {
    title: (p) => p.nom,
    meta: (p) => 'Bien inscrit' + (p.section ? ` - ${p.section}` : ''),
    text: ['nom', 'section', 'no_section'],
  },
  'zone-tampon': {
    title: (p) => `Zone tampon ${p.id || ''}`,
    meta: () => 'Zone tampon',
    text: ['id'],
  },
  'cites-minieres': {
    title: (p) => p.nom,
    meta: (p) => joinNotNull([p.commune_1, p.commune_2, p.commune_3]) || 'Cite miniere',
    text: [
      'nom',
      'nom_2',
      'commune_1',
  ...
  batis: {
    title: (p) => p.denomination || p.nom,
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Bati minier',
    text: [
      'denomination',
      'nom',
      'commune_1',
  ...
  cavaliers: {
    title: (p) => 'Cavalier' + (p.id_unesco ? ` ${p.id_unesco}` : ''),
    meta: (p) => joinNotNull([p.commune_1, p.commune_2, p.commune_3]) || 'Cavalier',
    text: [
      'commune_1',
      'commune_2',
      'commune_3',
  ...
  'espace-neonaturel': {
    title: (p) => p.nom,
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Espace neo-naturel',
    text: ['nom', 'commune_1', 'commune_2', 'element', 'objet'],
  },
  terrils: {
    title: (p) => p.nom || `Terril ${p.no_terril || ''}`,
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Terril',
    text: ['nom', 'no_terril', 'commune_1', 'commune_2', 'compagnie', 'element', 'objet'],
  },
  'puits-de-mines': {
    title: (p) =>
      p.fosse ? `Fosse ${p.fosse}${p.fosse_alias ? ` (${p.fosse_alias})` : ''}` : 'Puits',
    meta: (p) => p.commune || 'Puits de mine',
    text: ['fosse', 'fosse_alias', 'puits', 'commune', 'compagnie', 'concession', 'id'],
  },
  'communes-mbm': {
    title: (p) => p.nom,
    meta: (p) =>
      'Commune' + (p.population ? ` - pop. ${Number(p.population).toLocaleString('fr-FR')}` : ''),
    text: ['nom', 'insee'],
  },
  epci: { title: (p) => p.nom, meta: (p) => 'EPCI', text: ['nom', 'code_siren'] },
  departements: { title: (p) => p.nom, meta: (p) => 'D\u00e9partement', text: ['nom', 'code'] },
  'zt-cavaliers': {
    title: (p) => p.nom || 'Cavalier (ZT)',
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Cavalier (zone tampon)',
    text: ['nom', 'commune_1', 'commune_2', 'commune_3', 'commune_4', 'id_troncon'],
  },
  'zt-cites-minieres': {
    title: (p) => p.nom,
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Cite miniere (zone tampon)',
    text: ['nom', 'nom_2', 'commune_1', 'commune_2', 'compagnie'],
  },
  'zt-espaces-neonaturels': {
    title: (p) => p.nom,
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Espace neo-naturel (zone tampon)',
    text: ['nom', 'commune_1', 'commune_2'],
  },
  'zt-terrils': {
    title: (p) => p.nom || 'Terril (ZT)',
    meta: (p) => joinNotNull([p.commune_1, p.commune_2]) || 'Terril (zone tampon)',
    text: ['nom', 'nom_usuel', 'commune_1', 'commune_2'],
  },
  'zt-parvis-agricoles': {
    title: (p) => `Parvis agricole ${p.id || ''}`,
    meta: (p) => p.qualite_vue || 'Parvis agricole',
    text: ['id', 'qualite_vue', 'vue_sur'],
  },
};

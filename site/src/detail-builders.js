...
import { rawHtml, normalizeText } from 'leaflet-atlas';

export function createDetailBuilders(helpers) {
  const { buildDetail, buildReverseLinksSection, sourceRow, getReverseLinks, eachLayerFeature } =
    helpers;

  return {
    'bassin-minier': (p) =>
      buildDetail(p.nom || 'Bassin minier', 'bassin-minier', [
        {
    ...
    'bien-inscrit': (p) => {
      ...
      const groups = [
        ...
        {
          label: 'Caracteristiques',
          rows: [p.surface_ha && ['Surface', `${p.surface_ha} ha`]],
        },
      ];
      const rl =
        reverseLinks && p.no_element
          ? buildReverseLinksSection(
              reverseLinks.elements[String(p.no_element)],
              'Elements rattaches'
            )
          : null;
    ...
    'zone-tampon': (p) =>
      buildDetail('Zone tampon', 'zone-tampon', [
        {
          label: 'Identification',
          rows: [p.id && ['ID', p.id]],
        },
    ...
    batis: (p) =>
      buildDetail(p.denomination || p.nom || 'Bati minier', 'batis', [
        {
          label: 'Localisation',
          rows: [p.commune_1 && ['Commune', communeLinks(p, 'commune_1', 'commune_2')]],
        },
        {
          label: 'Identification',
          rows: [
        ...
        {
          label: 'Protection',
          rows: [p.protection && p.protection !== 'non' && ['Protection', p.protection]].concat(
            mhRows(p)
          ),
        },
        {
          ...
          rows: [
            ...
            (p.nom || p.compagnie || p.periode || p.proprietaire || p.protection) &&
              sourceRow(dataGouvSources.mbm),
          ],
        },
      ]),
    cavaliers: (p) => {
      const cl = communeLinks(
        p,
        'commune_1',
        'commune_2',
        'commune_3',
      ...
      return buildDetail('Cavalier minier', 'cavaliers', [
        ...
        {
          label: 'Caracteristiques',
          rows: [p.longueur_m && ['Longueur', `${Math.round(p.longueur_m)} m`]],
        },
        {
          label: 'Identification',
          rows: [
    ...
    'espace-neonaturel': (p) =>
      buildDetail(p.nom || 'Espace neo-naturel', 'espace-neonaturel', [
        {
          label: 'Localisation',
          rows: [p.commune_1 && ['Commune', communeLinks(p, 'commune_1', 'commune_2')]],
        },
    ...
    terrils: (p) => {
      ...
      const groups = [
        ...
        {
          label: 'Caracteristiques',
          rows: [p.forme && ['Forme', p.forme]],
        },
      ];
      const rl =
        reverseLinks && p.no_terril
          ? buildReverseLinksSection(reverseLinks.terrils[p.no_terril], 'Vue depuis')
          : null;
      if (rl) groups.push(rl);
      groups.push({ label: 'Liens', rows: [sourceRow(dataGouvSources.patrimoine)] });
    ...
    'communes-mbm': (p) => {
      ...
      const rl =
        reverseLinks && p.nom
          ? buildReverseLinksSection(
              reverseLinks.communes[normalizeText(p.nom)],
    ...
    epci: (p) => {
      ...
      const groups = [
        {
          ...
          rows: [
            ...
            deptNames.length && [
              'D\u00e9partement(s)',
              rawHtml(deptNames.map((n) => deptLink(n)).join(', ')),
            ],
          ],
        },
      ];
      const rl =
        reverseLinks && p.nom
          ? buildReverseLinksSection(reverseLinks.epcis[normalizeText(p.nom)], 'Communes membres')
          : null;
      if (rl) groups.push(rl);
    ...
    departements: (p) => {
      ...
      const groups = [
        {
          label: 'Identification',
          rows: [p.code && ['Code', p.code]],
        },
      ];
      const rlEpci =
        reverseLinks && p.nom
          ? buildReverseLinksSection(reverseLinks.departements[normalizeText(p.nom)], 'Contenu')
          : null;
    ...
    'zt-cavaliers': (p) => {
      ...
      return buildDetail(p.nom || 'Cavalier (zone tampon)', 'zt-cavaliers', [
        ...
        {
          label: 'Identification',
          rows: [p.id_troncon && ['Troncon', p.id_troncon]],
        },
        { label: 'Liens', rows: [sourceRow(dataGouvSources.mbm)] },
      ]);
    },
    ...
    'zt-espaces-neonaturels': (p) =>
      buildDetail(p.nom || 'Espace neo-naturel (zone tampon)', 'zt-espaces-neonaturels', [
        {
          label: 'Localisation',
          rows: [
            p.commune_1 && ['Commune', communeLinks(p, 'commune_1', 'commune_2', 'commune_3')],
    ...
    'zt-terrils': (p) => {
      ...
      const rl =
        reverseLinks && p.id
          ? buildReverseLinksSection(reverseLinks.terrils[String(p.id)], 'Vue depuis')
          : null;
      if (rl) groups.push(rl);
      groups.push({ label: 'Liens', rows: [sourceRow(dataGouvSources.mbm)] });
      return buildDetail(nom, 'zt-terrils', groups);
    },
    'zt-parvis-agricoles': (p) =>
      buildDetail('Parvis agricole', 'zt-parvis-agricoles', [
        {
          label: 'Identification',
          rows: [p.id && ['ID', p.id]],
        },
          ...
          rows: [
            p.brgm &&
              /^https?:\/\//.test(p.brgm) && [
                'Fiche BRGM',
                rawHtml(
                  `<a href="${encodeURI(p.brgm)}" target="_blank" rel="noopener">Voir la fiche BRGM<span class="sr-only"> (s'ouvre dans un nouvel onglet)</span><span class="cross-link-icon" aria-hidden="true"> \u2197</span></a>`
                ),
              ],

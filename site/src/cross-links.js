...
export const crossLinkHandlers = {
  commune: (app, value) => {
    ...
    app.showFeatureDetail(
      'communes-mbm',
      app.findFeatureIndex('communes-mbm', layer),
      layer.feature.properties
    );
    app.highlightFeature(layer, 'communes-mbm', {
      style: { weight: 4, fillOpacity: 0.5, color: '#e57373' },
    });
  },

  epci: (app, value) => {
    const found = app.findLayerByProperty(
      'epci',
      (p) => p.nom && normalizeText(p.nom) === normalizeText(value)
    );
    if (!found) return;
    ...
    app.highlightFeature(found, 'epci', {
      style: { weight: 4, fillOpacity: 0.2, color: '#7E57C2' },
    });
  },
  ...
  departement: (app, value) => {
    ...
    app.showFeatureDetail(
      'departements',
      app.findFeatureIndex('departements', found),
      found.feature.properties
    );
    app.highlightFeature(found, 'departements', {
      style: { weight: 4, fillOpacity: 0.1, color: '#FF8F00' },
    });
  },
  ...
  terril: (app, value) => {
    ...
    for (const lid of ['terrils', 'zt-terrils']) {
      found = app.findLayerByProperty(
        lid,
        (p) => String(p.no_terril) === value || String(p.id) === value
      );
      if (found) {
        foundLayerId = lid;
        break;
    ...
    app.showFeatureDetail(
      foundLayerId,
      app.findFeatureIndex(foundLayerId, found),
      found.feature.properties
    );
  },
  ...
  element: (app, value) => {
    ...
    app.showFeatureDetail(
      first.layerId,
      app.findFeatureIndex(first.layerId, first.layer),
      first.layer.feature.properties
    );
    for (const m of matches) {
